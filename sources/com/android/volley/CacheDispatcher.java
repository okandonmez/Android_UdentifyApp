...
public class CacheDispatcher extends Thread {
    ...
    private static class WaitingRequestManager implements NetworkRequestCompleteListener {
        ...
        public void onResponseReceived(Request<?> request, Response<?> response) {
            ...
            if (waitingRequests != null) {
                if (VolleyLog.DEBUG) {
                    VolleyLog.m6v("Releasing %d waiting requests for cacheKey=%s.", Integer.valueOf(waitingRequests.size()), cacheKey);
                }
        ...
        public synchronized void onNoUsableResponseReceived(Request<?> request) {
            ...
            if (!(waitingRequests == null || waitingRequests.isEmpty())) {
                if (VolleyLog.DEBUG) {
                    VolleyLog.m6v("%d waiting requests for cacheKey=%s; resend to network", Integer.valueOf(waitingRequests.size()), cacheKey);
                }
                ...
                try {
                    ...
                    VolleyLog.m4e("Couldn't add request to queue. %s", iex.toString());
        ...
        private synchronized boolean maybeAddToWaitingRequests(Request<?> request) {
            ...
            synchronized (this) {
                ...
                if (this.mWaitingRequests.containsKey(cacheKey)) {
                    ...
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m3d("Request for cacheKey=%s is in flight, putting on hold.", cacheKey);
                    }
                } else {
                    this.mWaitingRequests.put(cacheKey, null);
                    request.setNetworkRequestCompleteListener(this);
                    if (VolleyLog.DEBUG) {
                        VolleyLog.m3d("new request, sending to network %s", cacheKey);
                    }
                    z = false;
                }
            }
    ...
    public void run() {
        if (DEBUG) {
            VolleyLog.m6v("start new dispatcher", new Object[0]);
        }
