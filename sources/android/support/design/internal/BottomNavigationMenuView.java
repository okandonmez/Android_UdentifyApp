...
public class BottomNavigationMenuView extends ViewGroup implements MenuView {
    ...
    public BottomNavigationMenuView(Context context, AttributeSet attrs) {
        ...
        this.mInactiveItemMaxWidth = res.getDimensionPixelSize(C0012R.dimen.design_bottom_navigation_item_max_width);
        this.mInactiveItemMinWidth = res.getDimensionPixelSize(C0012R.dimen.design_bottom_navigation_item_min_width);
        this.mActiveItemMaxWidth = res.getDimensionPixelSize(C0012R.dimen.design_bottom_navigation_active_item_max_width);
        this.mItemHeight = res.getDimensionPixelSize(C0012R.dimen.design_bottom_navigation_height);
        ...
        this.mOnClickListener = new C00131();
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        for (i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != 8) {
                child.measure(MeasureSpec.makeMeasureSpec(this.mTempChildWidths[i], ErrorDialogData.SUPPRESSED), heightSpec);
                child.getLayoutParams().width = child.getMeasuredWidth();
                totalWidth += child.getMeasuredWidth();
            }
        }
        setMeasuredDimension(View.resolveSizeAndState(totalWidth, MeasureSpec.makeMeasureSpec(totalWidth, ErrorDialogData.SUPPRESSED), 0), View.resolveSizeAndState(this.mItemHeight, heightSpec, 0));
    }
