...
abstract class HeaderScrollingViewBehavior extends ViewOffsetBehavior<View> {
    ...
    public boolean onMeasureChild(CoordinatorLayout parent, View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        ...
        if (childLpHeight == -1 || childLpHeight == -2) {
            ...
            if (header != null) {
                ...
                parent.onMeasureChild(child, parentWidthMeasureSpec, widthUsed, MeasureSpec.makeMeasureSpec((availableHeight - header.getMeasuredHeight()) + getScrollRange(header), childLpHeight == -1 ? ErrorDialogData.SUPPRESSED : Integer.MIN_VALUE), heightUsed);
                return true;
            }
        }
        return false;
    ...
    final int getOverlapPixelsForOffset(View header) {
        return this.mOverlayTop == 0 ? 0 : MathUtils.clamp((int) (getOverlapRatioForOffset(header) * ((float) this.mOverlayTop)), 0, this.mOverlayTop);
    }
