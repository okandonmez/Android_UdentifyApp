    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (sizeDependentWidgetsCount > 0) {
            ...
            for (i = 0; i < sizeDependentWidgetsCount; i++) {
                widget = (ConstraintWidget) this.mVariableDimensionsWidgets.get(i);
                child = (View) widget.getCompanionWidget();
                if (!(child == null || (child.getMeasuredWidth() == widget.getWidth() && child.getMeasuredHeight() == widget.getHeight()))) {
                    child.measure(MeasureSpec.makeMeasureSpec(widget.getWidth(), ErrorDialogData.SUPPRESSED), MeasureSpec.makeMeasureSpec(widget.getHeight(), ErrorDialogData.SUPPRESSED));
                    if (this.mMetrics != null) {
                        metrics = this.mMetrics;
                        metrics.additionalMeasures++;
                    }
